...
def get_repo_files(api_url, token, ignore_patterns=None, look_patterns=None):
    ...
    if response.status_code == 200:
        ...
        for item in repo_contents:
            if item['type'] == 'file' and not should_ignore(item['path'], ignore_patterns, look_patterns):
                file_response = requests.get(item['download_url'], headers=headers)
                if file_response.status_code == 200:
                    files.append(f'"{item["path"]}": "{file_response.text}"\n\n')
                time.sleep(0.1)  # Delay to avoid hitting GitHub API rate limit
            elif item['type'] == 'dir':
                files.extend(get_repo_files(item['_links']['self'], token, ignore_patterns, look_patterns))
    return files

def main():
    parser = argparse.ArgumentParser(description="Fetch all files from a GitHub repo, with support for ignoring or specifically looking for files based on patterns.")
    ...
    parser.add_argument("-i", "--ignore", action='append', help="Regex pattern for files to ignore. Can be used multiple times.")
    parser.add_argument("-l", "--look", action='append', help="Regex pattern for files to specifically look for. Ignores others not matching the pattern. Can be used multiple times.")
    ...
    ignore_patterns = compile_patterns(args.ignore, default_patterns=[r'LICENSE', r'.*\.lock$'])
